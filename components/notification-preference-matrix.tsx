...
import type { getPreferences, UpdatablePreferences } from "@/lib/actions/notifications";
...
/** The Event/App/Email preference grid -- shared between its home at Settings -> Notifications
 *  and the persistent panel on the notifications feed itself, so the feed's own page can double
 *  as where a seller actually goes to quiet something down, not just a link elsewhere. */
export function NotificationPreferenceMatrix({ prefs }: { prefs: Awaited<ReturnType<typeof getPreferences>> }) {
  return (
    <Card>
